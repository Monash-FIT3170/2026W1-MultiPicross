...
export function MultiplayerGame() {
  ...
  async function startAuthGame(width: number, height: number) {
    ...
    try {
      const res = await apiFetch("/multiplayer/start", {
        method: "POST",
  ...
  async function startGuestGame(width: number, height: number) {
    ...
    try {
      const res = await apiFetch(
        `/multiplayer/guest-puzzle?width=${width}&height=${height}`,
      );
      if (!res.ok) {
        setPhase({ kind: "error", message: await parseApiError(res) });
        return;
  ...
  function handleFill(row: number, col: number) {
    ...
    if (game.isGuest) {
      ...
      void (async () => {
        try {
          const res = await apiFetch("/multiplayer/action", {
            method: "POST",
  ...
  async function handleAbandon() {
    ...
    if (!phase.game.isGuest) {
      try {
        await apiFetch("/multiplayer/abandon", { method: "POST" });
      } catch {
        /* ignore */
      }
    }
