...
import { randomPKCECodeVerifier, calculatePKCECodeChallenge, randomState, randomNonce, buildAuthorizationUrl, authorizationCodeGrant, } from "openid-client";
import { db } from "../db/client.js";
import { accounts, identities, loginAttempts, refreshTokens, } from "../db/schema.js";
import { isUniqueViolation } from "../db/errors.js";
import { env } from "../env.js";
import { hashPassword, verifyPassword, signAccessToken, signRefreshToken, signRoomToken, verifyRefreshToken, hashToken, refreshExpiresAt, } from "./helpers.js";
import { REFRESH_COOKIE, setAuthCookies, clearAuthCookies, newCsrfToken, } from "./cookies.js";
import { csrf } from "./csrf.js";
import { getOidcConfig, oidcRedirectUri, oidcScopes, oidcProviderId, } from "./oidc.js";
...
const HandleBody = v.object({
    handle: v.pipe(v.string(), v.minLength(3, "Handle must be at least 3 characters"), v.maxLength(20, "Handle must be at most 20 characters"), v.regex(/^[a-zA-Z0-9_-]+$/, "Handle may only contain letters, numbers, underscores and hyphens")),
});
function schema(s) {
...
async function issueSession(c, accountId) {
    ...
    if (Math.random() < PRUNE_SAMPLE_RATE) {
        pruneRefreshTokens().catch((err) => console.warn("refresh_tokens prune failed:", err));
    }
...
async function isLoginLocked(username) {
    const since = new Date(Date.now() - LOGIN_LOCKOUT_WINDOW_MS);
    const [row] = await db
        .select({ count: count() })
        .from(loginAttempts)
        .where(and(eq(loginAttempts.username, username), gte(loginAttempts.attemptedAt, since)));
...
auth.get("/callback", describeRoute({
    tags: ["Auth"],
    ...
    if (errorParam) {
        ...
        return c.redirect(`/auth/error?code=${encodeURIComponent(errorParam)}`, 302);
    }
    ...
    try {
        ...
        if (err instanceof AuthClaimError) {
            ...
            console.warn(`${err.message}. Claims present: ${Object.keys(claims).join(", ")}`);
            return c.redirect("/auth/error?code=missing_anchor", 302);
        }
        throw err;
    }
...
auth.post("/handle", requireAuth, csrf, describeRoute({
    tags: ["Auth"],
...
auth.post("/login", describeRoute({
    ...
    if (Math.random() < PRUNE_SAMPLE_RATE) {
        pruneLoginAttempts().catch((err) => console.warn("login_attempts prune failed:", err));
    }
...
auth.post("/refresh", csrf, describeRoute({
    tags: ["Auth"],
...
auth.post("/logout", csrf, describeRoute({
    tags: ["Auth"],
...
auth.get("/me", requireAuth, describeRoute({
    tags: ["Auth"],
...
auth.post("/room-token", requireAuth, describeRoute({
    tags: ["Auth"],
    summary: "Mint a short-lived token for joining a multiplayer room",
    description: "Proves the caller's identity to the gameserver without exposing the session cookie to it.",
    responses: {
        200: {
