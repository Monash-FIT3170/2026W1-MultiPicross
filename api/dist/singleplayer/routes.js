...
const GuestPuzzleQuery = v.object({
    width: v.pipe(v.string(), v.transform((s) => parseInt(s, 10)), v.check((n) => !Number.isNaN(n), "Must be a valid number"), v.integer(), v.minValue(1)),
    height: v.pipe(v.string(), v.transform((s) => parseInt(s, 10)), v.check((n) => !Number.isNaN(n), "Must be a valid number"), v.integer(), v.minValue(1)),
});
...
function computeElapsed(elapsedSeconds, lastResumedAt) {
    if (!lastResumedAt)
        return elapsedSeconds;
    return (elapsedSeconds + Math.floor((Date.now() - lastResumedAt.getTime()) / 1000));
}
const sp = new Hono();
sp.get("/sizes", describeRoute({
    tags: ["Singleplayer"],
...
sp.get("/guest-puzzle", describeRoute({
    tags: ["Singleplayer"],
    ...
    if (!result.success)
        return c.json({ error: result.error.map((i) => i.message).join(", ") }, 400);
}), async (c) => {
    ...
    const [puzzle] = await db
        ...
        .limit(1);
    if (!puzzle)
        return c.json({ error: "No puzzle of that size" }, 404);
    return c.json({
        id: puzzle.id,
        width: puzzle.width,
...
sp.get("/active", requireAuth, describeRoute({
    tags: ["Singleplayer"],
    ...
    const { sub: accountId } = c.get("jwtPayload");
    const [row] = await db
        .select({
        id: spCompletions.id,
        ...
        .where(and(eq(spCompletions.accountId, accountId), eq(spCompletions.state, "active")));
    if (!row)
...
sp.post("/start", requireAuth, csrf, describeRoute({
    tags: ["Singleplayer"],
    ...
    const completedRows = await db
        ...
        .from(spCompletions)
        .where(and(eq(spCompletions.accountId, accountId), eq(spCompletions.state, "completed")));
    const completedIds = completedRows.map((r) => r.puzzleId);
    const sizeFilter = and(eq(nonograms.width, width), eq(nonograms.height, height));
    const excludeFilter = completedIds.length > 0
    ...
    await db.transaction(async (tx) => {
        ...
        if (existing) {
            await tx
                ...
                .set({
                state: "abandoned",
                elapsedSeconds: computeElapsed(existing.elapsedSeconds, existing.lastResumedAt),
                lastResumedAt: null,
            })
...
sp.post("/action", requireAuth, csrf, describeRoute({
    tags: ["Singleplayer"],
    ...
    const result = await db.transaction(async (tx) => {
        const [gameRow] = await tx
            ...
            .for("update");
        if (!gameRow)
            return { status: 404, body: { error: "No active game" } };
        ...
        const confirmedFilled = gameRow.confirmedFilled;
        const crosses = gameRow.crosses;
        const revealedEmpty = gameRow.revealedEmpty;
        const mistakeCross = gameRow.mistakeCross;
        const solution = gameRow.solution;
        ...
        if (kind === "cross") {
            if (solution[idx] === 1) {
                ...
                const newLives = gameRow.livesLeft - 1;
                if (newLives <= 0) {
                    const finalElapsed = computeElapsed(gameRow.elapsedSeconds, gameRow.lastResumedAt);
                    await tx
                        .update(spCompletions)
                        .set({
                        state: "failed",
                ...
                const isComplete = solution.every((v, i) => v === 0 || newConfirmedFilled[i]);
                if (isComplete) {
                    const finalElapsed = computeElapsed(gameRow.elapsedSeconds, gameRow.lastResumedAt);
                    await tx
                        .update(spCompletions)
...
sp.post("/abandon", requireAuth, csrf, describeRoute({
    tags: ["Singleplayer"],
    ...
    const finalElapsed = computeElapsed(completion.elapsedSeconds, completion.lastResumedAt);
    await db
        .update(spCompletions)
        .set({
        state: "abandoned",
...
sp.get("/history", requireAuth, describeRoute({
    tags: ["Singleplayer"],
